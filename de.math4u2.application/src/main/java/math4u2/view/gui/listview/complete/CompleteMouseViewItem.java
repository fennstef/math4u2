...
/**
 * Darsteller der Mauskoordinate in dem CompleteViewContainer
 * 
 * @author Fenn Stefan
 * @see CompleteViewContainer
 */
public class CompleteMouseViewItem extends AbstractListViewItem {
	private JLabel fnLabel;
	private DefinitionField px;
	private DefinitionField py;

	public CompleteMouseViewItem(UserFunction model, ListViewInterface parent,
			Broker broker) {
	...
	protected void onceInit() {
		...
		try {
			...
			ExceptionManager.doError("Fehler beim Initialisieren der Mauskoordinaten-Sicht",e);
		}
		...
		panel2.add(new JLabel(Images.CURSOR));
		panel2.add(fnLabel);		
	...
	public void reInit() {
		...
		try {
			px.setMathModel(getModel().getRelationContainer().getObjectByName("x"));
			py.setMathModel(getModel().getRelationContainer().getObjectByName("y"));
		} catch (ObjectNotInRelationException e) {
			e.printStackTrace();
		}
		px.refresh();
