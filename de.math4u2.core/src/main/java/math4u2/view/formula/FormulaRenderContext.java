...
import static math4u2.util.io.file.FileResolver.resolve;
...
public class FormulaRenderContext {
	...
	/**
	 * Formel-Stil <i>Small</i>. Kleine Version der Formel, z.B. für Inidices
	 * oder Exponenten.
	 */
	public static final byte FONTSIZE_S = 2;
	/**
	 * Formel-Stil <i>Small Small</i>. Kleinste Ausführung der Formel, z.B. für
	 * Indices in Exponenten, usw.
	 */
	public static final byte FONTSIZE_SS = 3;

	/**
	 * Faktor, mit dem die Schriftgröße mutlipliziert werden muss, um auf eine
	 * <code>Mathematical Unit</code> (Mathematische Einheit) zu kommen. Dies
	 * ist die kleinste Einheit im Formelsatz und wird beispielsweise für die
	 * Größe von Zwischenräumen benutzt.
	 */
	public static final float MU = 1f / 16f;

	/** Die NAmen aller integrierten mathemathischen Funktionen */
	private static final String[] BUILTIN_FUNCTIONS = new String[] { "sin",
			"cos", "tan", "arcsin", "arccos", "arctan", "e", "log", "ln" };
	...
	/**
	 * Höhe der Mittelachse gegenüber der Grundlinie, z.B. für Bruchstriche
	 */
	...
	/**
	 * Saklierungsfaktor für große Operatoren, z.B. für Integral, Summe, Produkt
	 */
	...
	static {
		try {
			symbols.read(resolve(
					"math4u2/view/formula/unicodeSymbols.properties",
					FormulaRenderContext.class));
		} catch (IOException e) {
		...
		symbol_bold_italic = new Font("Lucida Sans", Font.ITALIC + Font.BOLD,
				12);
	}// static
	...
	private static Font loadSpecialFont(String filePath) {
		try {
			FileInputStream fis;
			fis = new FileInputStream(resolve(filePath,
					FormulaRenderContext.class));
	...
	/**
	 * Gibt den standardmäigen <code>FontRenderContext</code> zurück. Dieser
	 * wird beispielsweise benötigt, um die Vektor-Umrisse einer Schrift zu
	 * bekommen oder die Dimensionen zu berechnen, die eine bestimmte
	 * Zeichenkette in einer bestimmten Schrift auf dem Bildschirm einnehmen
	 * wird. Außerdem gibt der <code>FontRenderContext</code> an, ob die
	 * Darstellung mit Kantenglättung und/oder auf Subpixel-Basis erfolgt.
	 * 
	...
	public final int getDefaultFontSize() {
		...
		// int screenSize = (int) Math.round(DEFAULT_FONT_SIZE * screenRes /
		// 72.0);
		// return screenSize;
	...
	/**
	 * Gibt ein Feld mit den implementierten Standardfunktionen zurück. Dies ist
	 * beispielsweise für die Darstellung wichtig, um entscheiden zu können, wie
	 * bestimmte Schlüsselwörter dargestellt werden sollen.
	 * 
	...
	/**
	 * Gibt die Höhe der Bruchstrichlinie gegenüber der Grundlinie in <i>MU</i>
	 * zurück.
	 * 
	...
	/**
	 * Gibt den Faktor zurück, mit dem große Operatoren, (z.B. Integral, Summe,
	 * Produkt) skaliert werden sollen.
	 * 
	...
	/**
	 * Gibt den Faktor für einen bestimmten Formelstil (<code>FONTSIZE_D</code>,
	 * <code>FONTSIZE_T</code>, <code>FONTSIZE_S</code>,
	 * <code>FONTSIZE_SS</code>) zurück, mit dem die Schriftgröße multipliziert
	 * werden muss, um die reale Schriftgröße zu erhalten.
	 * 
	 * @param relSize
	 *            Formelstil
	...
	/**
	 * Verkleinert einen gegebenen Formelstil um eine Stufe. Die Zahl der Stufen
	 * ist begrenzt, so dass die Verkleinerung nur bis zu einer gewissen Stufe
	 * stattfindet.
	 * 
	 * @param relSize
	 *            Formelstil
	...
	/**
	 * Gibt das Symbol für einen bestimmtes Kürzel (<code>token</code>) zurück.
	 * Damit können einfach Sonderzeichen über deren entsprechende Kürzel
	 * angesprochen werden.
	 * 
	 * @param token
	 *            Kürzel für das das Symbol zurückgegeben werden soll
