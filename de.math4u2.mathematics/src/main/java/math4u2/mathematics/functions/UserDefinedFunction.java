...
/**
 * Grundgerüst für selbstdefinierte Funktionen in einer Xml-Lektion. In der
 * Xml-Datei wird festgelegt, welche Methode mit neuen Inhalten gefüllt werden
 * soll.
 * 
 * @author Fenn Stefan
 */
public class UserDefinedFunction extends Function implements HasGraph {
	...
	/**
	 * Mit dieser Membervariable kann der Benutzer sich Zwischenergebnisse
	 * merken.
	 */
	public Object cache;
	...
	public GraphInterface getGraph(DrawAreaInterface da, UserFunction f) {
		if (f.getArity() == 0) {
			this.f = f;			
			return getViewFactory().getGraphFactory().createSimpleGraph(da, this, f);
		} else if (f.getArity() == 1) {
			return getViewFactory().getGraphFactory()
					.createFunctionGraph(da, f);
		} else {
			throw new RuntimeException("Graph von " + f
		...
		public void run() {
			...
			while (true) {
				try {
					try {
						...
						while (!propagateChange
							...
							if (!propagateChange) {
								...
								long sleepTime = millis - (curTime - startTime)
										- 10;
								if (sleepTime < 10)
									break;
								sleep(sleepTime);
					...
					try {
						synchronized (this) {
							((HasBroker) MainWindow.get()).getBroker()
									.propagateChange(UserDefinedFunction.this);
