...
public class MatrixDiffFunction extends MatrixSumDiffStandardFunction {

	public MatrixDiffFunction() {
		name = "MatrixDiff";
		operatorName = "-";
		operatorNiceName = " - ";
	}
	...
	double[][] evalArray(MatrixDoubleResult arg1, MatrixDoubleResult arg2 ) throws MathException{
		...
		if (arg2.rowDim != rowDim)
			throw new MathException(
					"Unterschiedliche Zeilenzahl bei Differenz von Matrizen oder Vektoren");
		if (arg2.colDim != colDim)
			throw new MathException(
					"Unterschiedliche Spaltenzahl bei Differenz von Matrizen oder Zeilenvektoren");

		diffValueArray = new double[rowDim][colDim];
		double[][] sum1 = arg1.valueArray;
		double[][] sum2 = arg2.valueArray;

		for (int r = 0; r < rowDim; r++) {
			for (int c = 0; c < colDim; c++) {
				diffValueArray[r][c] = sum1[r][c] - sum2[r][c];
			}
		}
		return diffValueArray;
	}
	...
	public TermNode buildDeriveTerm(TermNode[] arguementTerms,
		...
		try {
			return new TermNodeFunct(
					(Function) (broker.getObject("MatrixDiff")),
