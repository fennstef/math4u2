 ...
 */
public class CompleteStretchViewItem extends AbstractListViewItem {
	...
	public CompleteStretchViewItem(UserFunction model, ListViewInterface parent,
			Broker broker) {
	...
	protected void onceInit() {
		...
		try {
			...
			begin = new DefinitionField(name + Broker.SEPERATOR + "start",
					getMathObject().getRelationContainer().getObjectByName(
							"start"), broker);
			end = new DefinitionField(name + Broker.SEPERATOR + "ende",
					getMathObject().getRelationContainer().getObjectByName(
							"ende"), broker);
		} catch (ObjectNotInRelationException e) {
			ExceptionManager.doError("Fehler beim Initialisieren der Strecken-Sicht",e);
		}
	...
	public void reInit() {
		...
		try {
			...
			ExceptionManager.doError("Fehler beim Erneuern der Strecken-Sicht",e);
		}//catch
