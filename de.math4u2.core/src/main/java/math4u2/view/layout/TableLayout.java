...
public class TableLayout implements java.awt.LayoutManager2,
		...
		while (iterator.hasNext()) {
			...
			if (entry.component == component)
				return new TableLayoutConstraints(entry.col1, entry.row1,
						entry.col2, entry.row2, entry.hAlign, entry.vAlign);
		}
	...
	public void insertColumn(int i, double size) {
		...
		if ((i < 0) || (i > columnSpec.length))
			throw new IllegalArgumentException("Parameter i is invalid.  i = "
					+ i + ".  Valid range is [0, " + columnSpec.length + "].");
	...
	public void insertRow(int i, double size) {
		...
		if ((i < 0) || (i > rowSpec.length))
			throw new IllegalArgumentException("Parameter i is invalid.  i = "
					+ i + ".  Valid range is [0, " + rowSpec.length + "].");
	...
	public void deleteColumn(int i) {
		...
		if ((i < 0) || (i >= columnSpec.length))
			throw new IllegalArgumentException("Parameter i is invalid.  i = "
					+ i + ".  Valid range is [0, " + (columnSpec.length - 1)
					+ "].");

		// Copy columns
		double column[] = new double[columnSpec.length - 1];
		System.arraycopy(columnSpec, 0, column, 0, i);
		System.arraycopy(columnSpec, i + 1, column, i, columnSpec.length - i
				- 1);
	 ...
	 * method returns. See <code>setRow</code>. There must be at least two
	 * rows in order to delete a row.
	 * 
	 * @param i
	 *            zero-based index of column to delete
	 * 
	...
	public void deleteRow(int i) {
		...
		if ((i < 0) || (i >= rowSpec.length))
			throw new IllegalArgumentException("Parameter i is invalid.  i = "
					+ i + ".  Valid range is [0, " + (rowSpec.length - 1)
					+ "].");
	...
	protected void calculateSize(Container container) {
			...
			if ((columnSpec[counter] == PREFERRED)
				...
				while (iterator.hasNext()) {
					...
					if ((entry.col1 == counter) && (entry.col2 == counter)) {
						Dimension p = (columnSpec[counter] == PREFERRED) ? entry.component
								.getPreferredSize()
			...
			if ((rowSpec[counter] == PREFERRED)
				...
				while (iterator.hasNext()) {
					...
					if ((entry.row1 == counter) && (entry.row2 == counter)) {
						Dimension p = (rowSpec[counter] == PREFERRED) ? entry.component
								.getPreferredSize()
								: entry.component.getMinimumSize();
		...
		for (counter = 0; counter < numColumn; counter++)
			...
			if ((columnSpec[counter] > 0.0) && (columnSpec[counter] < 1.0)) {
				...
				columnSize[counter] = (int) (columnSpec[counter]
						* relativeWidth + 0.5);
	...
	protected class Entry extends TableLayoutConstraints {
		 ...
		 *            equal to this TableLayoutConstraints.
		 * 
