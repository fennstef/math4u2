...
public class Straight extends AbstractAffineObject {
	...
	public Straight(UserFunction point, UserFunction direction, Color color,
			int lineStyle, boolean isVisible, Broker broker,
			ViewFactoryInterface viewFactory) {
		super("gerade", null, color, lineStyle, isVisible, broker, viewFactory);
		this.startPoint = point;
		this.vector = direction;
		this.broker = broker;
	} // Konstruktor

	public Straight(MathObject startPoint, MathObject direction, Broker broker,
			ViewFactoryInterface viewFactory) {
	...
	/**
	 * Verknüpft die Gerade mit seinen Beziehungspartnern (parts)
	 * 
	 * @param s
	...
	public static void register(MathObject s, List partsWithPath, List regList,
		...
		if (!(s instanceof Straight))
			throw new IllegalArgumentException(
					"Das Objekt muß eine Gerade sein. Ist aber " + s.getClass());
		...
		ra[0].setShortName(Relation.FIRST, "punkt");
	...
	public GraphInterface getGraph(DrawAreaInterface da, UserFunction f) {
		return viewFactory.getGraphFactory().createStraightGraph(da, f);
	} // getGraph
	...
	public void setName(String newName) {
		if (name == null)
			...
			throw new RuntimeException("Gerade mit Name " + name
	...
	public ListViewItemInterface getCompleteView(UserFunction f,
		...
		return viewFactory.createCompleteStraightViewItem(f, alv, broker);
	} // getCompleteView
	...
	public Object operator_punkt(Object[] values) {
		try {
	...
	public Object operator_set_punkt(Object[] values) {
		Object obj = values[0];
		TermNode tm;
	...
	public Class returnType_punkt(MethodContext mc) {
